...
class HomeAssistant():
    ...
    async def __hass_websocket(self):
        ...
        while self.hue.event_loop.is_running():
            try:
                self.__last_id = 10
                self._callbacks = {}
                async with self.http_session.ws_connect('%s://%s/api/websocket' % (
                        protocol, self._host), verify_ssl=False) as ws:
                        ...
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            if msg.data == 'close cmd':
                                ...
                                if data['type'] == 'auth_required':
                                    ...
                                    await send_msg({"type": "get_states"}, callback=self.__all_states)
                                    # request all area, device and entity registry
                                    await send_msg({"type": "config/area_registry/list"}, 
                                        callback=self.__receive_area_registry)
                                    await send_msg({"type": "config/device_registry/list"}, 
                                        callback=self.__receive_device_registry)
                                    await send_msg({"type": "config/entity_registry/list"}, 
    ...
    async def __post_data(self, endpoint, data):
        ...
        headers = {"Authorization": "Bearer %s" %
                   self.hue.config.hass_token, "Content-Type": "application/json"}
        async with self.http_session.post(url, headers=headers,
                                          json=data, verify_ssl=False) as response:
            return await response.json()
