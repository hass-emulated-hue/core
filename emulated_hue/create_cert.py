...
def generate_selfsigned_cert(cert_file, key_file, config):
    ...
    root_cert = x509.CertificateBuilder().subject_name(
        ...
        x509.SubjectKeyIdentifier(b"hash").from_public_key(root_key.public_key()), critical=False
    ).add_extension(
        x509.AuthorityKeyIdentifier(
            key_identifier=b"keyid,issuer",
            authority_cert_issuer=None,
            authority_cert_serial_number=None).from_issuer_public_key(root_key.public_key()),
        critical=False
    ).add_extension(
        x509.KeyUsage(digital_signature=True, content_commitment=False, key_encipherment=True, data_encipherment=False,
