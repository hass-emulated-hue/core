...
class HueApi():
    ...
    async def async_setup(self):
        ...
        self.http_site = web.TCPSite(self.runner, self.config.host_ip_addr, self.config.http_port)
        self.https_site = web.TCPSite(self.runner, self.config.host_ip_addr, self.config.https_port,
        ...
        try:
            ...
            _LOGGER.error(
                "Failed to create HTTP server at port %d: %s", self.config.http_port, error)
        try:
            await self.https_site.start()
            _LOGGER.info("Started HTTPS webserver on port %s", self.config.https_port)
        except OSError as error:
            _LOGGER.error(
                "Failed to create HTTPS server at port %d: %s", self.config.https_port, error)
    ...
    async def post_auth(self, request, request_data):
        ...
        response = [{"success": {"username": const.HUE_USERNAME}}]
        if request_data.get("generateclientkey"):
            response[0]["success"]["clientkey"] = const.HUE_CLIENT_KEY
    ...
    async def put_light_state(self, request, request_data):
        ...
        response = await self.__create_hue_response(request.path, request_data)
        return web.json_response(response)
    ...
    async def update_group(self, request, request_data):
        ...
        if "stream" in local_group:
            if local_group["stream"].get("active"):
                ...
                _LOGGER.info(
                    "Start Entertainment mode for group %s - params: %s", group_id, request_data)
                if not self.streaming_api:
                    self.streaming_api = EntertainmentThread(
                        self.hue, local_group)
                    self.streaming_api.start()
                if not local_group["stream"].get("owner"):
                    local_group["stream"]["owner"] = const.HUE_USERNAME
                ...
                _LOGGER.info(
                    "Stop Entertainment mode for group %s - params: %s", group_id, request_data)
        ...
        await self.config.set_storage_value("groups", local_groups)
        response = await self.__create_hue_response(request.path, request_data)
        return web.json_response(response)
    ...
    async def get_localitems(self, request):
        ...
        result = await self.__get_local_items(itemtype)
        return web.json_response(result)
    ...
    async def get_localitem(self, request):
        ...
        itemtype = request.path.split("/")[3]
        items = await self.__get_local_items(itemtype)
    ...
    async def update_localitem(self, request, request_data):
        ...
        item_id = request.match_info['item_id']
        itemtype = request.path.split("/")[3]
        local_items = await self.__get_local_items(itemtype)
        if not item_id in local_items:
            return web.Response(status=302)
        update_dict(local_items, request_data)
        await self.config.set_storage_value(itemtype, local_items)
        response = await self.__create_hue_response(request.path, request_data)
        return web.json_response(response)
    ...
    async def get_full_state(self, request):
        ...
        json_response = {
            ...
            "schedules": await self.__get_local_items("schedules"),
            "rules": await self.__get_local_items("rules"),
            "scenes": await self.__get_local_items("scenes"),
            "resourcelinks": await self.__get_local_items("resourcelinks"),
    ...
    async def unknown_request(self, request):
        ...
        if request.method in ["PUT", "POST"]:
            request_data = await request.json()
            _LOGGER.warning("Invalid request: %s --> %s",
    ...
    async def __light_action(self, entity, request_data):
        ...
        service = const.HASS_SERVICE_TURN_ON if power_on else const.HASS_SERVICE_TURN_OFF
        if power_on:
    ...
    async def __entity_to_json(self, entity):
        ...
        if ((entity_features & const.HASS_SUPPORT_BRIGHTNESS)
            ...
            retval["state"].update(
                {
                    ...
                    const.HUE_ATTR_XY: entity_attr.get(const.HASS_ATTR_XY_COLOR, [0, 0]),
        ...
        elif (entity_features & const.HASS_SUPPORT_BRIGHTNESS) and (entity_features & const.HASS_SUPPORT_COLOR):
            ...
            retval["state"].update({
                ...
                const.HUE_ATTR_CT: entity_attr.get(
                    const.HASS_ATTR_COLOR_TEMP, 0)
            })
    ...
    async def __create_hue_response(self, request_path, request_data):
        """Create success responses for all received keys."""
        request_path = request_path.replace(f"/api/{const.HUE_USERNAME}", "")
        json_response = []
    ...
    async def __get_local_items(self, itemtype="scenes"):
        """Get all items in storage of given type (scenes etc.)."""
        return await self.config.get_storage_value(itemtype, {})
    ...
    async def __get_bridge_config(self, full_details=False):
        ...
        if full_details:
            result.update({
                ...
                "whitelist": {const.HUE_USERNAME: {"name": "HASS BRIDGE"}},
                "zigbeechannel": 25,
            })
        return result
